...
def symplectic_to_pauliword(
    symplectic_array: np.array, coeff: float = 1.0
) -> QubitOperator:
...
def build_G_matrix(list_qubit_operators: List, n_qubits: int) -> np.array:
    ...
    Based on page 11 of https://arxiv.org/pdf/1701.08213.pdf).
...
def build_E_matrix(G_matrix: np.array) -> np.array:
    """Function to build E matrix.

    Based on equation 60 of https://arxiv.org/pdf/1701.08213.pdf (pg11).
...
def gf2_gaus_elim(gf2_matrix: np.array) -> np.array:
    ...
    while row_i < m_rows and col_j < n_cols:
        ...
        gf2_matrix_rref[:, col_j:] = np.bitwise_xor(
            gf2_matrix_rref[:, col_j:], Om_j_dependent_rows_flip
        )
...
def apply_pi4_rotation(
    rot_pauilword_A: QubitOperator, Pauliword_B: QubitOperator
) -> QubitOperator:
    """Apply rotation.
    R Pb R^dagger
...
def Clifford_operator_reduction_to_X(list_symmetry_generators) -> Tuple[list, list]:
    """Map list of symmetry operators to single qubit Pauli X operators using Clifford rotations.
    ...
    while list_symmetry_generators:
        ...
        if len(sym_paulis) == 1 and "".join(sym_paulis) == "X":
            ...
            if "".join(sym_paulis) == "X" * len(sym_qno):
                ...
                # apply to generators
                list_symmetry_generators = [
                    apply_pi4_rotation(pauli_Y_rot, op)
                    for op in list_symmetry_generators
                ]
            else:
            ...
            Z_Y_single_qop_sig_qno, Z_Y_single_coeff = zip(
                *op_with_one_Z_or_Y.terms.items()
            )
            ...
            if "Z" in Z_Y_single_paulis:
                Z_indices = set(
                    q_ind
                    for q_ind, sigma in zip(Z_Y_single_qNos, Z_Y_single_paulis)
                    if sigma == "Z"
                )
                ...
                raise ValueError(
                    f"{op_with_one_Z_or_Y} does not contain single qubit Z or Y"
                )

            # build operator to map op_with_one_Z_or_Y to single qubit Z (only differs on
            P_rot_to_single_X = QubitOperator(
                " ".join(
                    [
                        f"{pauli_needed}{qNo_to_map_to_X}"
                        if qNo == qNo_to_map_to_X
                        else f"{s}{qNo}"
                        for qNo, s in zip(Z_Y_single_qNos, Z_Y_single_paulis)
                    ]
                ),
                Z_Y_single_coeff[0],
            )
            ...
            # apply to generators
            list_symmetry_generators = [
                apply_pi4_rotation(P_rot_to_single_X, op)
                for op in list_symmetry_generators
            ]
...
def get_rotated_operator_and_generators(
    pauli_hamiltonian: QubitOperator,
) -> Tuple[QubitOperator, list, list, list]:
    ...
    single_X_generators, clifford_rotations = Clifford_operator_reduction_to_X(
        symmetry_generators
    )
...
def find_sector_brute_force(
    rotated_hamiltonian: QubitOperator, symmetry_generators: List
) -> List:
    """Return the tapered Hamiltonian.
    ...
    for measurement_assignment in itertools.product(
        [+1, -1], repeat=len(symmetry_generators)
    ):
        ...
        generator_ind_and_meas_val = dict(
            zip(symmetry_generator_qubit_indices, measurement_assignment)
        )
        ...
        for h_op in rotated_hamiltonian:
            ...
            Pauliword_reduced = " ".join(
                [
                    f"{Pauli}{qubit_relabel[qNo]}"
                    for qNo, Pauli in zip(qNos, paulis)
...
def find_ground_sector_using_input_state(
    rotated_hamiltonian: QubitOperator,
    symmetry_generators_pre_rotatation: List,
    ...
    """
    qubit_state_Z_mesaure = -1 * (
        (2 * qubit_state) - 1
    )  # plus and minus one for Z measurement
    ...
    for sym_op_ind, symm_op in enumerate(symmetry_generators_pre_rotatation):
        ...
        if not "".join(sym_paulis) == "Z" * len(sym_qno):
            raise ValueError(
                f"Currently function only works for Z generators! {symm_op} not valild"
            )
        generator_ind_and_meas_val[sym_op_ind] = (
            np.prod(np.take(qubit_state_Z_mesaure, sym_qno)) * coeff[0]
        )
    ...
    symmetry_generator_signs = [
        coeff for term in symmetry_generators for coeff in term.terms.values()
    ]
...
def get_ground_tapered_H_using_HF_state(
    pauli_hamiltonian: QubitOperator,
    HF_qubit_state: np.array,
    ...
    (
        ...
        single_X_generators,
        clifford_rotations,
    ) = get_rotated_operator_and_generators(pauli_hamiltonian)

    H_tapered = find_ground_sector_using_input_state(
        H_rotated,
        symmetry_generators,
        HF_qubit_state,
        single_X_generators,
        check_correct=check_tapering,
    )
...
if __name__ == "__main__":
    ...
    H = [
        QubitOperator("Z0", random.uniform(5, 10)),
    ...
    single_X_generators, cliff_rotations = Clifford_operator_reduction_to_X(
        symmetry_generators
    )
    ...
    H_tapered_ground = find_ground_sector_using_input_state(
        H_rotated,
        symmetry_generators,
        JW_array,
        single_X_generators,
    ...
    H_tapered_ground_mat = get_sparse_operator(
        reduce(lambda x, y: x + y, H_tapered_ground)
    )
    ...
    H_tap = get_ground_tapered_H_using_HF_state(
        reduce(lambda x, y: x + y, H), JW_array, check_tapering=True
    )
